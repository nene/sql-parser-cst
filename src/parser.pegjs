...
alter_default_privileges_stmt
  = kw:(ALTER __ DEFAULT __ PRIVILEGES __)
    clauses:((default_acl_for_role_clause / default_acl_in_schema_clause) __)*
...
default_acl_for_role_clause
  = kw:(FOR __) roleKw:((ROLE / USER) __) roles:list$role_specification {
    return loc({ type: "default_acl_for_role_clause", forKw: read(kw), roleKw: read(roleKw), roles });
  }

default_acl_in_schema_clause
  = kw:(IN __) schemaKw:(SCHEMA __) schemas:list$ident {
    return loc({ type: "default_acl_in_schema_clause", inKw: read(kw), schemaKw: read(schemaKw), schemas });
  }
...
error_bigquery
  = kw:ERROR {
    return loc({ type: "error_bigquery", errorKw: kw });
  }
...
for_range
  = reverseKw:(REVERSE __)? from:(expr __) ".." to:(__ expr) by:(__ for_by_clause)? {
    return loc({
      type: "for_range",
...
assert_stmt
  = kw:(ASSERT __) condition:expr message:(__ assert_message)? {
    return loc({
      type: "assert_stmt",
      assertKw: read(kw),
      condition: read(condition),
...
assert_message
  = &bigquery x:as_clause$string_literal { return x; }
  / &plpgsql x:comma_clause$expr { return x; }
...
alter_action_drop_publication_objects
  = kw:(DROP __) publicationObjects:list$publication_object {
      return loc({
 ...
 * To add new rule run:                                                                 *
 *                                                                                      *
 *   yarn add:template 'comma_clause$some_name'                                         *
 * ------------------------------------------------------------------------------------ *
 */
comma_clause$__template__
  = "," expr:(__ __template__) {
    return loc({
      type: "comma_clause",
      expr: read(expr),
    });
